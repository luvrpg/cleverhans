...
class cifar10vgg:
    ...
    @staticmethod
    def normalize(x_train_, x_test_):
        # this function normalize inputs for zero mean and unit variance
        # it is used when training a model.
        # Input: training set and test set
        # Output: normalized training set and test set according to the trianing set statistics.
        mean = np.mean(x_train_, axis=(0, 1, 2, 3))
        std = np.std(x_train_, axis=(0, 1, 2, 3))
        x_train_ = (x_train_ - mean) / (std + 1e-7)
        x_test_ = (x_test_ - mean) / (std + 1e-7)
    ...
    def train(self, model):
        ...
        model_saver = keras.callbacks.ModelCheckpoint("vgg_models", monitor='val_loss',
        ...
        historytemp = model.fit_generator(datagen.flow(x_train, y_train,
                                          ...
                                          epochs=maxepoches,
                                          validation_data=(x_test, y_test), callbacks=[reduce_lr, model_saver],
                                          verbose=1)
